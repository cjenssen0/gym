...
class HovorkaCambridgeBase(gym.Env):
    ...
    def __init__(self):
        ...
        if bg_init_flag == 'random':
            self.init_basal = np.random.choice(np.linspace(
                init_basal_optimal-2, init_basal_optimal, 10))
        ...
        self.state = np.concatenate(
            [np.repeat(initial_bg_norm, self.stepsize), initial_insulin, [t_norm]])
    ...
    def step(self, action):
        ...
        for i in range(self.simulation_time):
            ...
            insulin_rate = action + \
                (self.meal_indicator[self.num_iters] * (180/self.bolus))

            # Setting the carb and insulin parameter in the model
            self.integrator.set_f_params(
                insulin_rate, self.meals[self.num_iters], self.P)
        ...
        self.state = np.concatenate(
            [bg_norm, self.insulin_history[-4:], [t_norm]])
    ...
    def render(self, mode='human', close=False):
        ...
        if mode == 'rgb_array':
            ...
            super(HovorkaCambridgeBase, self).render(
